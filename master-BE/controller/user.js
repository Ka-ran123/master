...
const { fileUpload, profilePicDeleteOnCloudinary } = require("../utility/cloudinary");
...
const getUsers = async (req, res) => {
  try {
    ...
    const totalUsers = await countUser(userRole);
    const maxPage =
      totalUsers <= limitDoc ? 1 : Math.ceil(totalUsers / limitDoc);
    ...
    const skip = (pageCount - 1) * limitDoc;
    const userList = await findAllUser(userRole, {
      offset: skip,
      limit: limitDoc,
    });
